...
class RoleInfoCommand : LorittaCommand(arrayOf("roleinfo", "taginfo"), CommandCategory.DISCORD) {
    ...
    suspend fun root(context: DiscordCommandContext, locale: BaseLocale, legacyLocale: LegacyBaseLocale) {
        var role = context.discordGuild!!.getRoleById("")
        val argument = context.args[0]

        if (context.discordMessage.mentionedRoles.firstOrNull() != null) {
            role = context.discordMessage.mentionedRoles.firstOrNull()
        }
        else if (context.args.isNotEmpty() && argument.isValidSnowflake() && context.event.guild!!.getRoleById(argument) != null) {
            role = context.event.guild!!.getRoleById(argument)
        }
        else if (context.event.guild!!.getRolesByName(argument, true) != null) {
            role = context.event.guild!!.getRolesByName(argument, true)[0]
        }
        else {
            role = null
            LoriReply(locale["commands.discord.roleinfo.roleNotFound"], Constants.ERROR)
            return
        }
            if (role != null) {
